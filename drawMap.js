...
if (!polygonDrawer) {
    ...
    document.getElementById('drawMap').textContent = 'Draw district';
    document.getElementById('drawMap').style.backgroundColor = '#28a745';
}
}
...
polygonDrawer = null;
document.getElementById('drawMap').textContent = 'District Saved';
document.getElementById('drawMap').style.backgroundColor = '#d3d3d3';        
});


function clearMap() {
// Clear drawn items from the feature group
drawnItems.clearLayers();

// Also remove other layers (markers, etc.) but keep base tiles
map.eachLayer(function (layer) {
if (layer instanceof L.Marker || (layer instanceof L.Polygon && !drawnItems.hasLayer(layer)) || 
    layer instanceof L.Polyline || layer instanceof L.Rectangle || layer instanceof L.Circle) {
...
try {
// Convert Leaflet polygon to turf.js format if needed
...
// Simple bounding box overlap check as fallback
// This is a basic implementation - for production use turf.js intersect
...
// Helper function to get bounds from polygon
...
// Helper function to calculate overlap area between two bounding boxes
...
// Helper function to calculate area of bounding box
...
// Show districts if they're not already visible
if (!districtsVisible) {
...
document.getElementById('addLayer').textContent = 'Hide districts';
}
...
districtsData.features.forEach(function(districtFeature) {
    ...
    // If overlap is above threshold (e.g., 70%), consider it correct
...
alert(`Score: ${correctMatches}/${totalDrawn} correct (${score}%)`);
}
...
document.getElementById('addLayer').textContent = 'Show districts';
}
});
