...
const cityConfig = {
    austin: {
        ...
        resultsText: `Under the Texas GOP proposed redistricting plan, Austin's District 37 triggers the prospect of Austin Democratic Reps. Casar and Lloyd Doggett facing each other in a primary for the area's lone remaining blue district.`
    },
    houston: {
        mapId: 'houston-map',
    ...
    checkResults() {
        ...
        if (distanceMiles < 1.24) { // ~2km converted to miles
            this.updateResultsText(`
                ✅ <strong>Excellent!</strong> Your district placement is very accurate!
                <br><small>Distance from correct center: ${Math.round(distanceMiles * 100) / 100} miles</small>
                <br><br>${this.config.resultsText}
            `, '#28a745');
            this.showNotification('🎉 Great job! Very close to the actual boundaries!');
        } else if (distanceMiles < 3.11) { // ~5km converted to miles
            this.updateResultsText(`
                ⚠️ <strong>Good effort!</strong> Your placement is close to the target area.
                <br><small>Distance from correct center: ${Math.round(distanceMiles * 100) / 100} miles</small>
                <br><br>${this.config.resultsText}
            `, '#ffc107');
        } else {
            this.updateResultsText(`
                ❌ <strong>Keep trying!</strong> Your district needs to be repositioned.
                <br><small>Distance from correct center: ${Math.round(distanceMiles * 100) / 100} miles</small>
                <br><br>${this.config.resultsText}
            `, '#dc3545');
        }
...
//         // Make a district layer draggable for adjustments
//     makeLayerDraggable(layer) {
//         let isDragging = false;
//         let dragStartLatLng = null;
//         let originalStyle = null;
...
//             // Store original style and apply dragging style
//             originalStyle = layer.options.style || layer.options;
//             layer.setStyle({
//                 ...originalStyle,
//                 opacity: 0.7,
//                 fillOpacity: 0.4,
...
//                     dashArray: '5, 5'
//                 },
//                 onEachFeature: (feature, newLayerInstance) => {
//                     const districtName = feature.properties.name || `District ${this.config.district}`;
//                     newLayerInstance.bindPopup(`
//                         <div style="text-align: center;">
//                             <strong>${districtName}</strong><br>
//                             <small>Double-click to remove</small><br>
//                             <small>Drag to adjust position</small>
...
//             // Replace in droppedLayers array
//             const layerIndex = this.droppedLayers.indexOf(layer);
//             if (layerIndex > -1) {
//                 this.droppedLayers[layerIndex] = newLayer;
//             }
            
//             // Make new layer draggable
//             this.makeLayerDraggable(newLayer);
...
//                 layer.setStyle({
//                     ...originalStyle,
//                     opacity: originalStyle.opacity || 0.8,
//                     fillOpacity: originalStyle.fillOpacity || 0.3,
//                     weight: originalStyle.weight || 3,
//                     dashArray: originalStyle.dashArray || null
//                 });
//             }
...
//             // Show adjustment notification
//             this.showNotification('✅ District position adjusted!', '#28a745');
